...
describe('toggleProductToCart', () =>{
    ...
    it('succeeds on toggle state', async () => {
        ...
        list = lists[0]
        expect(list.products).to.have.lengthOf(1)
        expect(list.products[0].state).to.equal('')
    ...
    it('fails on existing list', async () => {
        ...
        try {
            return await toggleProductToCart(listTestNoExistsId, contactTest.id, productTest.id)
        } catch (error) {
    ...
    it('fails on existing user', async () => {
        ...
        try {
            return await toggleProductToCart(listTest.id, userTestNoExistsId, productTest.id)
        } catch (error) {
            expect(error).to.be.instanceOf(Error)
    ...
    it('fails on existing product', async () => {
        ...
        try {
            return await toggleProductToCart(listTest.id, contactTest.id, productTestNoExistsId)
        } catch (error) {
            expect(error).to.be.instanceOf(Error)
    ...
    it('fails on empty listId', () => 
        expect(() => toggleProductToCart('', contactTest.id, productTest.id)).to.throw(Error, 'list id does not have 24 characters')
    )

    it('fails on empty userId', () =>
        expect(() => toggleProductToCart(listTest.id, '', productTest.id)).to.throw(Error, 'user id does not have 24 characters')
    )

    it('fails on empty productId ', () =>
        expect(() => toggleProductToCart(listTest.id, contactTest.id, '')).to.throw(Error, 'product id does not have 24 characters')
    )
