...
const toggleLikeProduct = require('./toggleLikeProduct')
...
describe('toggleLikeProduct', () =>{
    ...
    it('succeeds on toggle like', async () => {
        await toggleLikeProduct(listTest.id, contactTest.id, productTest.id)
        let lists = await List.find({})
        expect(lists).to.have.length(1)
        let list = lists[0]
        expect(list.products).to.have.lengthOf(1)
        expect(list.products[0].likes).to.have.lengthOf(1)
        expect(list.products[0].likes[0].toString()).to.equal(contactTest.id)
        ...
        list = lists[0]
        expect(list.products).to.have.lengthOf(1)
        expect(list.products[0].likes).to.have.lengthOf(0)
    ...
    it('fails on existing list', async () => {
        ...
        try {
            return await toggleLikeProduct(listTestNoExistsId, contactTest.id, productTest.id)
        } catch (error) {
    ...
    it('fails on existing user', async () => {
        ...
        try {
            return await toggleLikeProduct(listTest.id, userTestNoExistsId, productTest.id)
        } catch (error) {
            expect(error).to.be.instanceOf(Error)
    ...
    it('fails on existing product', async () => {
        ...
        try {
            return await toggleLikeProduct(listTest.id, contactTest.id, productTestNoExistsId)
        } catch (error) {
            expect(error).to.be.instanceOf(Error)
    ...
    it('fails on empty listId', () => 
        expect(() => toggleLikeProduct('', contactTest.id, productTest.id)).to.throw(Error, 'list id does not have 24 characters')
    )

    it('fails on empty userId', () =>
        expect(() => toggleLikeProduct(listTest.id, '', productTest.id)).to.throw(Error, 'user id does not have 24 characters')
    )

    it('fails on empty productId ', () =>
        expect(() => toggleLikeProduct(listTest.id, contactTest.id, '')).to.throw(Error, 'product id does not have 24 characters')
    )
