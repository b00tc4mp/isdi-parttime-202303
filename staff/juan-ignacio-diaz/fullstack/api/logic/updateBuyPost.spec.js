...
describe('updateBuyPost' , () =>{
    ...
    it('succeeds on update buy post', () => {
        const userTest = usersTest[0]
        const postTest = postsTest[2]

        return updateBuyPost(userTest.id, postTest.id)
            .then(() => Post.findById(postTest.id))
            .then(post => {
                expect(post).to.exist
                expect(post.author.toString()).to.equal(userTest.id)
    ...
    it('fails when user not exists', () => {
        ...
        return updateBuyPost(userTestNoExistsId, postTest.id)
            .catch(error => {
    ...
    it('fails when post not exists', () => {
        ...
        return updateBuyPost(userTest.id, postTestNoExistsId)
            .catch(error => {
                expect(error).to.be.instanceOf(Error)
    ...
    it('fails on empty user id', () => {
        ...
        expect(() => updateBuyPost('', postTest.id)).to.throw(Error, 'user id does not have 24 characters')
    })

    it('fails on empty post id', () => {
        const userTest = usersTest[0]

        expect(() => updateBuyPost(userTest.id, '')).to.throw(Error, 'post id does not have 24 characters')
    })
