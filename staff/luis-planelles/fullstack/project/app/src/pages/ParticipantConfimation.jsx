...
import retrieveParticipant from '../logic/retrieveParticipant';
...
const ParticipantConfirmation = () => {
  ...
  useEffect(() => {
    try {
      retrieveParticipant(missionId, participantId)
        .then((participant) => {
  ...
  return (
    <section className="fixed inset-0 flex items-center justify-center z-50">
      ...
      {mission  && (
        <div className="mb-4">
          <p className="text-lg">
            <strong>Hello {participant.name}.</strong> <br /> <br /> Your friend <strong>{mission.creatorName} </strong> 
          ...
          <p className="text-lg">
            ...
            on real time, and we will keep you informed through reports from 
            <a 
        ...
        <form className="space-y-4" onSubmit={handleConfirmParticipant}>
          ...
          <div>
            <p className="text-lg text-red-500">
              Sorry, this mission has ended. You can no longer participate.  
            </p>
              <Link to={`/mission/${missionId}`}> If you can wath the result of the mission</Link>
          </div>
        )}
      </div>
    </section>
