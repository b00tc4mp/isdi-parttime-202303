...
export default function MenuLayer({ onClose, items, isUserLoggedIn }) {
    ...
    return (
        <nav className="flex bg-gray-400/80 z-50 fixed top-0 left-0 w-full h-full backdrop-blur-lg uppercase">
            <ul className="px-4 self-center [&>*]:font-normal [&>*]:text-3xl">
                ...
                {items.map(item => (
                    <MenuItem
                        ...
                        to={item.link}
                        handleItemClick={item.click ? item.click : onClose}
                    >
                        {item.label}
                    </MenuItem>
                ))}
                ...
                <li className='text-lg text-gray-200 flex items-center pt-1 pr-1'>
                    <span className="ml-3 text-sm font-medium ">dark</span>
