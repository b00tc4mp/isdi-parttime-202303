    ...
    return <div className="w-full relative flex flex-col gap-[8px]">
        ...
        <div className="flex justify-between w-[100%]">
                ...
                {post && <>
                    ...
                    <div>
                        <div className="flex gap-[4px] items-center">
                            ...
                            {post.author.id === context.token && <span className="material-symbols-rounded icon-xs ">{post.visibility === 'private' ? 'lock' : 'language'}</span>}
                        </div>
            ...
            <div className="flex gap-[8px] items-center">
                {(context.token !== post.author.id && post.price > 0) && <a className="link" onClick={handleOpenBuyPost}>Buy post</a>}
                {context.token === post.author.id && <button className="secondary-button icon-button" onClick={handleOpenModal}><div className="icon-s-container"><span className="material-symbols-rounded icon-s pointer">more_vert</span></div></button>}
            </div>
        </div>
        ...
        <div className="w-full h-auto flex justify-between items-center">
            ...
            {<div className="flex gap-[8px]">
                ...
                <div className="icon-s-container" onClick={handleLikedPost}>
                    <span className={`material-symbols-rounded icon-s pointer ${post.likes && post.likes.includes(context.token) ? 'like-icon-filled' : ''}`} >favorite</span>
                </div>
            </div>}
        </div>
    </div>
