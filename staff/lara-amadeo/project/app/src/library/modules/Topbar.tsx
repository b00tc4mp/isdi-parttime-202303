    ...
    return <>
        <div className={`topbar-container ${className}`}>
            {level === 'first' && <>
                ...
                <div className={`${firstLevel?.menu && !firstLevel.chat ? 'topbar-label-icon-left' : ''} ${!firstLevel?.menu && firstLevel?.chat ? 'topbar-label-icon-right' : ''} ${!firstLevel?.menu && !firstLevel?.chat ? 'topbar-label' : ''}`}><p className='small-text-bold grey-700'>{firstLevel?.label}</p>
                </div>
