...
refreshScreen = () => {
    ...
    if (game) renderGame()
    if (game && playerTurn && !playerAction && megalodon.hp > 0) renderActionsMenu(megalodon)
    if (game && playerTurn && playerAction && megalodon.hp > 0){
        resolveAction(megalodon, kraken, playerAction)
        megalodon.reset('damage')
        renderAction(megalodon, kraken, playerAction)
    }    
    if (game && enemyTurn && kraken.hp > 0) {
    ...
    if (endGame && kraken.hp <= 0 && megalodon.hp > 0) {
        renderWonGame()
        megalodon.reset('hp')
        kraken.reset('hp')
    }
