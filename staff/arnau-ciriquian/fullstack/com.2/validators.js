...
function validatePasswordConfirm(password, passwordConfirm, explainOne = 'password confirmation', explainTwo ='password') {
    if (typeof passwordConfirm !== 'string') throw new TypeError(`${explain}confirmation is not a string`)
    if(!passwordConfirm.length) throw new ContentError(`${explainOne} confirmation is empty`)
    if (password !== passwordConfirm) throw new ContentError(`${explainOne} confirmation is different than ${explainTwo}`)
}
