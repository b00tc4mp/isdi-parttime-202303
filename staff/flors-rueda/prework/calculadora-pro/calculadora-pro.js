const getNumbers = (numbers = []) => {
  ...
  const addition = (numbers) => {
    ...
    return Number.isInteger(total) ? total : total.toFixed(3);
  };
  ...
  const calculator = (finalResults = []) => {
    ...
    if (stringNumbers.length > 0) {
      ...
      if (numbers.length === 1) {
        const squareRoot = Math.sqrt(numbers[0]);
        const total = Number.isInteger(squareRoot) ? squareRoot : squareRoot.toFixed(3);
        finalResults.push("Square root = " + total);
