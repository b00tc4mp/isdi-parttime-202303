...
const UserCard = ({ userInfo }) => {
    ...
    return (
        <div className="w-full p-4 border border-light400 bg-light500 rounded-lg shadow flex flex-row gap-5 md:flex-row-reverse justify-center md:mb-5">
            ...
            <Link to={`/profile/${userInfo.id}`} className="flex flex-col justify-center align-center h-full py-4 md:py-0">
                <img className={`bg-${userInfo.color} md:mt-3 w-16 w-16 md:w-18 md:h-18 rounded-full self-center`} src={`${avatars[userInfo.avatar]}`} alt="avatar" />
            </Link>
        </div>
    )
}
