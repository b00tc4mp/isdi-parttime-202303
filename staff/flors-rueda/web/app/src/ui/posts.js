...
/**const fill = (item) => {
  ...
  if(item.classList.contains('fav-button')) {
    const pathList = item.querySelector('.fav-button > path');
    pathList.forEach(path => path.setAttribute('d', favFillPath));
  }
  if(item.classList.contains('like-button')) {
    const pathList = item.querySelector('.like-button > path');
    pathList = path.setAttribute('d', likeFillPath);
  }
};
...
const unfill = (item) => {
  ...
  if(item.classList.contains('fav-button')) {
    const pathList = item.querySelectorAll('.fav-button > path');
    pathList.forEach(path => path.setAttribute('d', favUnfillPath));
  }
  if(item.classList.contains('like-button')) {
    const pathList = item.querySelectorAll('.like-button > path');
    pathList.forEach(path => path.setAttribute('d', likeUnfillPath));
  }
} */
...
const renderPost = (post, userAuth) => {
  ...
  html += `</div>
            ...
            <b>${post.fav}</b></div>
            <div class="post-like">
            <svg class="like-button  ${likeClass}n" xmlns="http://www.w3.org/2000/svg" viewBox="0 96 960 960"><path d="${likePath}"/></svg>
