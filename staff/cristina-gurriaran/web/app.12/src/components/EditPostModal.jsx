...
export default function EditpostModal({ onCancel , onPostUpdated, postId }) {
    ...
    function handleUpdatePost(event){
        ...
        try{
            updatePost(context.userId, postId, image, location, title, text)
            onPostUpdated()
    ...
    try{
        const { image, location, title, text } = retrievePost(context.userId, postId)
        ...
        return <section className='edit-post modal'>
            <form className="container" onSubmit={handleUpdatePost}>
                ...
                <textarea className="input" name="location" placeholder="location" defaultValue={location}></textarea>
                <textarea className="input" name="title" placeholder="title" defaultValue={title}></textarea>
