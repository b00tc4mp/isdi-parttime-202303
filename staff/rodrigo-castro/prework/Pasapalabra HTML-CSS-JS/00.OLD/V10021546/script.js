...
function countDown(){
  setIntervalID=setInterval(decrementTimer, 1000); //setInterval(función, intervalo de tiempo que queramos que se ejecute la función). Devuelve su ID.
  segundos=150;
  decrementTimer();
  }
...
function decrementTimer(){
  ...
  document.getElementById("js--timer").textContent=segundos;
  if(segundos==0){
...
const addEventListeners = () => {
  ...
  //const temporizer = document.querySelector(".seconds");
    ...
    if (userAnswers.length < questions.length) {
      if (input.value !== "" && input.value !== null && showQuestion.textContent !== null && showQuestion.textContent !== "" && numberOfQuestion < questions.length) {
        ...
        if (input.value.toLowerCase() === questions[numberOfQuestion].answer){
          document.getElementById(`letter${questions[numberOfQuestion].letter}`).classList.add('correct');
          acertsCounter++;
        } else {
          document.getElementById(`letter${questions[numberOfQuestion].letter}`).classList.add('incorrect');
        }
        numberOfQuestion++;
        if (userAnswers.length < questions.length) {
          showQuestion.textContent = questions[numberOfQuestion].question;
