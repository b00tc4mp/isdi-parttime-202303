...
const getIntroduction = () => {
  let namePlayer = prompt(
    "Por favor, introduce tu nombre para emprezar la partida."
  );
  if (!namePlayer) {
    alert("Nungún nombre ha sido introducido, por favor introduzca alguno.");
    return getIntroduction();
  }
  alert(
    `Perfecto ${namePlayer}, ahora antes de empezar la partida procederemos a explicar las reglas del juego.`
  );
  alert(
    `El juego consiste en:
    - El jugador, con ayuda de preguntas, tendrá que adivinar una palabra relacionada con cada una de las letras del alfabeto.
    - La palabra podra empezar con dicha letra o simplemente contener esta, pero traquilo que la propia pregunta lo indicará.
    - Las palabras deberán ser escritas sin tildes ni dieresis, y en caso de que se puedan escribir en femenino o masculino, simempre en masculino.
    - Si se acierta una palabra esta se guardará como acertada.
    - Si se escribe 'Pasapalabra', se deja en blancola casilla, o se pulsa 'Cancelar' se pasará de turno y la pregunta se guardará para la siguiente ronda.
    - Si se falla una pregunta la palabra se dará como fallada y no se guardará.
    - Las rondas serán infinitas.
    - El juego podrá ser terminado en todo momento introduciendo la palabra "END", en caso de terminar antes de que acaben las preguntas el jugador recibirá la cantidad de palabras acertadas, pero no aparecerá en el ranking.`
  );
  return namePlayer;
};
...
let rondas = 1;
const resolveUnansweredQuestions = (numberQuestion) => {
  rondas++;
  
  let QUL = unasnweredQuestions.length;
  if (QUL > 0) {
    alert(`Atención, vas a pasar a la siguiente ronda, ¡animo!`);
    for (let i = 0; i < QUL; i++) {
      let status = getQuestion(unasnweredQuestions, i, numberQuestion);
      if (status === null) return null;
      if (i === QUL - 1) unasnweredQuestions.splice(0, QUL);
    }
    if (QUL === 0) return;
    if (QUL > 0) return resolveUnansweredQuestions(numberQuestion);
  }
  return;
};

const getQuestion = (gameQuestions, letter, numberQuestion) => {
  const questionToAsk = [];
  const rightWords = [];
  let { question } = gameQuestions[letter].alternativeWords[numberQuestion];
  let { answer } = gameQuestions[letter].alternativeWords[numberQuestion];
  questionToAsk.push(question);
  rightWords.push(answer);

  let roundQuestion = prompt(
    `${questionToAsk}\n\nSi quiere pasar de turno y dejar la pregunta para la siguiente ronda introduzca 'Pasapalabra'.\nSi quiere salir del juego introduzca la palabra "End".`,
    "Pasapalabra"
  );

  if (!roundQuestion) {
    unasnweredQuestions.push(gameQuestions[letter]);
    alert("¡PASAPALABRA!");
    return;
  }
  roundQuestion = roundQuestion.toLowerCase();
  if (roundQuestion === "pasapalabra") {
    unasnweredQuestions.push(gameQuestions[letter]);
    alert("¡PASAPALABRA!");
    return;
  }
  if (roundQuestion === "end") {
    alert(
      `El número de palabras que has acertado es de ${guessedWords.length}.`
    );
    alert(
      "¡Gracias por jugar a nuestro juego de Pasapalabra, hasta pronto!"
    );
    return null;
  }
  if (roundQuestion === rightWords[0]) {
    guessedWords.push(roundQuestion);
    alert(
      `¡Enhorabuena, acertaste la palabra "${rightWords}"!\nTienes 1 punto.`
    );
    alert("¡PASAPALABRA!");
    return;
  }
  if (roundQuestion !== rightWords[0]) {
    wrongWords.push(roundQuestion);
...
const startGame = (numberQuestion) => {
  alert(
    `El juego empezará cuando se diga ¡Pasapalabra! Estás preparado? Empezemos...`
  );
  alert(`¡¡PASAPALABRA!!`);
  for (let letter = 0; letter < questions.length; letter++) {
    let status = getQuestion(questions, letter, numberQuestion);
    if (status === null) return null;
  }
  let status = resolveUnansweredQuestions(numberQuestion);
  if (status === null) return null;
  return;
};

const getResults = (players, namePlayer) => {
  players.push({ player: namePlayer, words: guessedWords.length });
  players.sort((a, b) => b.words - a.words);
  alert(`¡Enhorabuena, has acabado el juego de pasapalabra!`);
  alert(
    `El total de palabras acertadas es de ${guessedWords.length}.\nY el total de palabras falladas es ${wrongWords.length}.`
  );
  console.log(
    `El ranking de jugadores según las plabras que han acertado es de:`
  );
  for (let i = 0; i < players.length; i++) {
    console.log(
      `${i + 1}# El jugador ${players[i].player} con ${
        players[i].words
      } acertadas.`
    );
  }
  alert(
    `El ranking de jugadores por palabras acertadas aparecerá en la consola.`
  );
  if (players[0].player === namePlayer) {
    console.log(`¡Enhorabuena, has terminado en primera posicion!`);
    alert(`¡Enhorabuena, has terminado en primera posicion!`);
  }
  return;
};
...
const continuePlaying = () => {
  let question = confirm(
    'Una vez ya has acabado puedes volver a jugar el juego de Pasapalabra pero con palabras diferentes, te gustaría?\nSi pulsas "Aceptar" se seguirá el juego en las condiciones que te acabo de decir.\nSi pulsas "Cancelar" posrás salir del juego.'
  );
  if (question) return true;
  else {
    alert("¡Gracias por jugar a nuestro juego de Pasapalabra, hasta pronto!");
  }
};

alert("Hola! Bienvenido al juego de pasapalabra!");
let namePlayer = getIntroduction();
let partida = 0;

const pasapalabra = () => {
  let status = startGame(partida);
  if (status === null) return;

  const players = [
    { player: "Jesús", words: 4 },
    { player: "Pilar", words: 6 },
    { player: "María", words: 3 },
  ];

  getResults(players, namePlayer);
  let anotherRound = continuePlaying();
  if (anotherRound) {
    partida++;
    if (partida === 3) partida = 0;
    wrongWords.splice(0, wrongWords.length);
    unasnweredQuestions.splice(0, unasnweredQuestions.length);
