...
module.exports = (userId, postId, commentId) => {
    ...
    return (async () => {
        ...
        if (index < 0)
            throw new PropertyError(`comment with id ${commentId} not found in post with id ${postId}`)

        const comment = post.comments[index]

        if (comment.author.toString() !== userId)
            throw new PropertyError(`comment with id ${commentId} does not belong to user with id ${userId}`)
