...
const palabras =
    [
        ['a', ['anducir', 'CON LA A. Dicho de una supuesta criatura extraterrestre: Apoderarse de alguien'], ['alabeo', 'CON LA A. Comba de cualquier cuerpo o superficie'], ['afable', 'CON LA A. Agradable, dulce, suave en la conversación y el trato']],
        ...
        ['t', ['taceta','CON LA T. Caldero pequeño de cobre que sirve en los molinos de aceite para trasegarlo'],['tumbaburros','CON LA T. Repertorio en forma de libro o en soporte electrónico en el que se recogen, según un orden determinado, las palabras o expresiones de una o más lenguas, o de una materia concreta, acompañadas de su definición, equivalencia o explicación'],['tsunami','CON LA T. Ola gigantesca producida por un maremoto o una erupción volcánica en el fondo del mar']],
...
function numeroAleatorio(palabras) {
    return Math.floor(Math.random() * ((palabras.length - 1) - 1 + 1) + 1);
}

let questions = [];

function llenaQuestions(questions, palabras) {
for(let i in palabras){
    let seleccion=numeroAleatorio(palabras[i]);
    const pregunta= new Word(palabras[i][0],palabras[i][seleccion][0],palabras[i][seleccion][1]);
    questions.push(pregunta);
}
}

llenaQuestions(questions,palabras);
